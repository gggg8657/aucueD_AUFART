...
from .models.cGAN import Encoder, Decoder, Discriminator
...
from .models.decoders_AU_dec import Generator, Generator2, Generator3, AGenerator
...
from .models.encoders_enc import TDC,ResnetEncoder, ResnetEncoder_feat
class DECA(nn.Module):
    ...
    def _create_model(self, model_cfg):
        ...
        # self.D_detail = Generator3(latent_dim=self.n_detail+self.n_cond, au_dim=41, out_channels=1, out_scale=model_cfg.max_z, sample_mode = 'bilinear').to(self.device)
        self.D_detail = Generator().to(self.device) #cGAN decoder
        self.AU_D_detail = AGenerator(latent_dim=27*512, au_dim=41, out_channels=1, out_scale=model_cfg.max_z, sample_mode = 'bilinear').to(self.device)
        # resume model
    ...
    def encode(self, images, batch, use_detail=True):
        ...
        if use_detail:
            ...
            # with torch.no_grad():
            # print('use detail')
            _,expcode = self.emoca.encode(batch=batch, training=False)
        ...
        if use_detail:
            x,afn,main_cl= self.AUNet(images) # afn : [16,27,512] #codedict ['exp'] shape is [16,50]
            codedict['afn']=afn
            # codedict['afn'] = (afn >= 0.3).float() #condition vector
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        ## decode
        verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shapecode'], expression_params=codedict['expcode'], pose_params=codedict['posecode'])
        if self.cfg.model.use_tex:
            albedo = self.flametex(codedict['texcode'])
        else:
        ...
        if use_detail:
            ...
            uv_z_1 = self.D_detail(torch.cat([codedict['posecode'][:,3:], codedict['expcode'], codedict['detailcode']], dim=1))
            uv_z_2 = self.deca.AU_D_detail(codedict['afn'])
            uv_z = (uv_z_1 + uv_z_2)
            ...
            uv_shading = self.render.add_SHlight(uv_detail_normals, codedict['lightcode'])
