...
class ConditionalDiscriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.utils.spectral_norm(nn.Conv2d(6, 64, kernel_size=3, stride=2, padding=1)),  # 64 x 112 x 112
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1)),  # 128 x 56 x 56
            nn.InstanceNorm2d(128),
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1)),  # 256 x 28 x 28
            nn.InstanceNorm2d(256),
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1)),  # 512 x 28 x 28
            nn.InstanceNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True),
            nn.utils.spectral_norm(nn.Conv2d(512, 1, kernel_size=3, stride=1, padding=1))  # 1 x 28 x 28
            ,nn.Sigmoid()
        )
...
class AUConditionalDiscriminator(nn.Module):
    def __init__(self):
        super(AUConditionalDiscriminator, self).__init__()
        self.l1 = nn.Conv2d(27*512, 41, kernel_size=1)
        self.model = nn.Sequential(
            nn.utils.spectral_norm(nn.Conv2d(6, 64, kernel_size=3, stride=2, padding=1)),  # 64 x 112 x 112
            nn.LeakyReLU(0.2, inplace=True),
...
class Generator5(nn.Module):
    def __init__(self, latent_dim=100, out_channels=1, out_scale=0.01, sample_mode = 'bilinear'):
        ...
        self.out_scale = out_scale
        self.condition_embedding = nn.Linear(27*512, 128)
        # self.encoder = Encoder()
        # self.init_size = 32 // 4  # Initial size before upsampling
        self.lin = nn.Conv2d(27*512, 41, kernel_size=1)
        # self.l1 = nn.Sequential(nn.Linear(latent_dim, 128 * self.init_size ** 2))
        self.model = nn.Sequential(
            nn.Upsample(scale_factor=2),
            nn.Conv2d(256, 256, kernel_size=3, padding=1),  # 256 x 2 x 2
            nn.InstanceNorm2d(256),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Upsample(scale_factor=2),
            nn.Conv2d(256, 512, kernel_size=3, padding=1),  # 512 x 4 x 4
    ...
    def forward(self, noise, cond):
        ...
        cond_emb = cond_emb.view(cond_emb.size(0), -1, 1, 1)
        # cond = (cond>0.3).int().float()
        
        # # cond = self.lin(cond)
        out = torch.cat([noise, cond_emb], dim=1)
        # out = out.view(out.shape[0], 222, 1, 1)
        img = self.model(out)
        return img*0.01