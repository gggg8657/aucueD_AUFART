...
class Head(nn.Module):
    ...
    def forward(self, x, use_gnn=False):
        # AFG
        f_u = []
        for i, layer in enumerate(self.main_class_linears):
            f_u.append(layer(x).unsqueeze(1))
        f_u = torch.cat(f_u, dim=1)
        f_v_d = f_u.mean(dim=-2)
        if use_gnn == False:
            f_v_d = F.normalize(f_v_d, p=2, dim=-1)
            return f_v_d
        else:
            # FGG
            f_v_d2 = F.normalize(f_v_d, p=2, dim=-1)
            f_v = self.gnn(f_v_d)
            b, n, c = f_v.shape
            ...
            main_cls = (main_cl * main_sc.view(1, n, c)).sum(dim=-1)
            # return f_v_d, main_cls
            ...
            cl = torch.cat([main_cls, sub_cl], dim=-1)
            return f_v_d2, cl
...
class AFG(nn.Module):
    ...
    def forward(self, x, use_gnn=True):
        # x: b d c
        x = self.backbone(x)
        x = self.global_linear(x)
        if use_gnn:
            afn, main_cl = self.head(x, use_gnn)
            return x, afn, main_cl
        else:
            afn = self.head(x, use_gnn)
            return x, afn