...
# from .models.OpenGraphAU.model.ANFL import AFG, MEFARG
...
class Trainer(object):
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        ###--------------- training detail model
            ...
            # losses['photo_detail_mrf'] = self.mrf_loss(uv_texture_patch*uv_vis_mask_patch, uv_texture_gt_patch*uv_vis_mask_patch)*self.cfg.loss.mrf
            losses['photo_detail_mrf'] = self.mrf_loss(torchvision.transforms.Resize(256)(images), \
                torchvision.transforms.Resize(256)(predicted_detail_images))*self.cfg.loss.mrf #mrf from ALbedoGAN
            ...
            img_afn = self.AU_net(images)[1] #memory problem
            img_afn_ = img_afn+0.1
            img_afn = torch.round(img_afn_)
            ...
            rend_afn_ = rend_afn+0.1
            rend_afn = torch.round(rend_afn_)
            # rend_afn = torch.round(rend_afn*10, decimals=0)
            losses['AU_feature'] = F.cross_entropy(rend_afn, img_afn)
            # losses['au_feature_loss'] = self.au_feature_loss(uv_texture_patch*uv_vis_mask_patch, uv_texture_gt_patch*uv_vis_mask_patch)
