...
class Trainer(object):
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        ###--------------- training detail model
            ...
            uv_z = self.deca.D_detail(torch.cat([posecode[:,3:], expcode, codedict['detailcode']], dim=1), codedict['afn'])
            # render detail
            ...
            with autocast():
                ...
                losses['au_feature_loss'], losses['chin_loss'], losses['dimp_loss']= self.au_feature_loss(self.deca.AUNet(images), self.deca.AUNet(predicted_detail_images)) # ver 2
