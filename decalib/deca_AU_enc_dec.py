...
from .models.encoders_enc import ResnetEncoder, ResnetEncoder_feat, DetailAUTransformer
from .models.FLAME import FLAME, FLAMETex
from .models.decoders_AU import Generator, AGenerator
...
class DECA(nn.Module):
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        if use_detail:
            with torch.no_grad():
                uv_z_1 = self.D_detail(torch.cat([codedict['pose'][:,3:], codedict['exp'], codedict['detail']], dim=1))
            uv_z_2 = self.AU_D_detail(codedict['afn'])
            uv_z = (uv_z_1 + uv_z_2)
        ...
        if return_vis:
            ...
            visdict = {
                ...
                'final_images': predicted_detail_images
            }
            if self.cfg.model.use_tex:
                visdict['rendered_images'] = ops['images']

            return opdict, visdict['final_images'], visdict['shape_detail_images'], (uv_texture_gt - uv_texture).mean()
    ...
    def model_dict(self):
        return {
            ...
            'AU_D_detail': self.AU_D_detail.state_dict()
