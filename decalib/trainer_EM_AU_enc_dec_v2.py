...
from .utils import lossfunc_AU_enc_dec_v2 as lossfunc
...
class Trainer(object):
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        ###--------------- training detail model
            ...
            masks = masks[:,None,:,:]
            codedict['AU_features'], afn, _= self.deca.AUNet(images)
            codedict['afn'] = afn
            # codedict['encoded_AUF'] = self.deca.AUEncoder(codedict['AU_features'])
            codedict['global_feature'] = torch.cat([posecode[:,3:], expcode, codedict['detailcode']], dim=1)
            codedict['detail_input'] = self.deca.DAT(codedict['afn'], codedict['global_feature'])
            # uv_z = self.deca.D_detail(torch.cat([posecode[:,3:], expcode, codedict['detailcode'],codedict['afn'],codedict['encoded_AUF']], dim=1))
            uv_z = self.deca.D_detail(torch.cat([posecode[:,3:], expcode, codedict['detailcode'],codedict['afn']], dim=1))
            # render detail
            ...
            with autocast():
                ...
                losses['au_feature_loss'], losses['chin_loss'], losses['dimp_loss'],losses['au_class_loss']= self.au_feature_loss(self.deca.AUNet(images), self.deca.AUNet(predicted_detail_images)) # ver 2
