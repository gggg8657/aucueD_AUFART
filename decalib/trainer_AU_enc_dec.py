...
from .utils import lossfunc_AU_enc_dec as lossfunc
...
class Trainer(object):
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        if self.cfg.loss.shape_consistency or self.cfg.loss.detail_consistency:
            ...
            for key in ['tex', 'exp', 'pose', 'cam', 'light', 'images', 'afn']:
                code = codedict[key]
        ...
        ###--------------- training detail model
            ...
            losses['au_class_consistency_loss']= self.L2(self.deca.AUNet(images)[1], self.deca.AUNet(predicted_detail_images)[1])*300 #ver 3
