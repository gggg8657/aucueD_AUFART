...
class Trainer(object):
    ...
    def configure_optimizers(self):
        ...
        self.optimizer_D = torch.optim.Adam(self.deca.D.parameters(), lr=0.0004, betas=(0.5, 0.999))
    ...
    def load_checkpoint(self):
        ...
        if self.cfg.train.resume and os.path.exists(os.path.join(self.cfg.output_dir, 'model.tar')):
            checkpoint = torch.load(os.path.join('/mnt/sdb/DJ_Training/encoder_reuse_v3/models/', '00189000.tar'))
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        ###--------------- training detail model
            ...
            uv_z = self.deca.D_detail(torch.cat([posecode[:,3:].unsqueeze(-1).unsqueeze(-1).expand(-1,-1,1,1), expcode.unsqueeze(-1).unsqueeze(-1).expand(-1,-1,1,1), codedict['detailcode'].unsqueeze(-1).unsqueeze(-1).expand(-1,-1,1,1)], dim=1)) # v1
            ...
            # losses['photo_detail'] = ((uv_texture_patch*uv_vis_mask_patch - uv_texture_gt_patch*uv_vis_mask_patch)**2).mean()*self.cfg.loss.photo_D
            # losses['z_reg'] = torch.mean(uv_z**2)*self.cfg.loss.reg_z
            ...
            au_fe_loss = nn.MSELoss()
            losses['au_feature_loss'] = au_fe_loss(a, b) * 4.0 #2.0 not yet tested
    ...
    def fit(self):
        ...
        for epoch in range(start_epoch, self.cfg.train.max_epochs):
            ...
            for step in tqdm(range(iters_every_epoch), desc=f"Epoch[{epoch+1}/{self.cfg.train.max_epochs}]"):
                ...
                losses['D_loss']= self.discriminator_step(self.transform(opdict['images']), opdict['images'], final_image, losses,step)
                losses['G_loss'] =self.generator_step(self.transform(opdict['images']), final_image, losses,step)
