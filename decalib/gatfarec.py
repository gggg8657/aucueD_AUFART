...
from .models.decoders import Generator, AGenerator
...
from .models.GATE import GATE, TDC, TDC2
from .models.encoders_au import AUEncoder, AUEncoder2
...
class DECA(nn.Module):
    ...
    def _create_model(self, model_cfg):
        ...
        # self.E_detail_deca = AUEncoder().to(self.device)
        ...
        self.AU_Encoder_disp = AUEncoder2().to(self.device)
        ...
        if os.path.exists(model_path):
            ...
            util.copy_state_dict(self.TDC.state_dict(), checkpoint['TDC'])
            ...
            if 'D_detail' in checkpoint.keys():
                util.copy_state_dict(self.D_detail.state_dict(), checkpoint['D_detail'])
            try : util.copy_state_dict(self.AU_D_detail.state_dict(), checkpoint['AU_D_detail'])
            except : print("AU_D_DETAIL only for DEMOS")
    ...
    def encode(self, images_224,  scene_image=None, use_detail=True, use_gnn=False):
        ...
        x, parameters_224 = self.E_flame.forwardResNet(images_224) # why this line exist?
        
        full_rep = self.AU_Encoder(x)
        # print(full_rep.shape)           #[16,512] 
        full_rep = full_rep.unsqueeze(1)
        # print(full_rep.shape)           #[16,1,512] 
        # print(afn.shape)                #[16,27,512]
        feats = torch.concat([full_rep, afn], dim=1)
        # adj = torch.cat([adj, torch.ones((B, 1, 27)).to(self.device)], dim=1)
        # adj = torch.cat([adj, torch.ones((B, 28, 1)).to(self.device)], dim=2)
        # parameters = self.GATE(feats)
        # print(feats.shape)              # [16,28,512]
        parameters = self.TDC(afn, full_rep) # transformer learn about the.... AU and full_rep
        ...
        if use_detail:
            ...
            # detailcode = detailcode.reshape(16,512)
            # detailcode = detailcode.unsqueeze(1)
            
            # # AU_disp = self.AU_Encoder_disp(images_224)
            # # print(AU_disp.shape)
            # # print(detailcode.shape)
            # # print(afn.shape)
            # detailcode = self.TDC2(afn, detailcode)
    ...
    # @torch.no_grad()
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, vis_au=False, return_vis=True, use_detail=True, 
                use_gnn=False,render_orig=False, original_image=None, tform=None):
        ...
        if self.cfg.model.use_tex:
            opdict['albedo'] = torch.ones([1, 3, self.uv_size, self.uv_size], device=images.device) * 0.5
        else:
            opdict['albedo'] = albedo        

        if use_detail:
            with torch.no_grad():
                uv_z_b = self.D_detail_deca(torch.cat([codedict['pose'][:, 3:], codedict['exp'], codedict['detail']], dim=1))
            uv_z_au = self.AU_D_detail(codedict['afn'])
            uv_z = uv_z_b + uv_z_au
