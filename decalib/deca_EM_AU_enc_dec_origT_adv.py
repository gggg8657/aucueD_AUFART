...
# from .models.encoders_enc import 
from .models.FLAME import FLAME, FLAMETex
from .models.decoders_AU_dec_adv import Generator, Generator2, Generator3, Generator4,ConditionalDiscriminator
...
from .models.encoders_enc import TDC,ResnetEncoder, ResnetEncoder_feat, Encoder
class DECA(nn.Module):
    ...
    def _create_model(self, model_cfg):
        ...
        # self.DAT = TDC(detail_dim=2048, au_dim=512, nhid=256, noutput=128, nfeat=512,
        #                nheads=4, batchsize=16,
        #                alpha=0.2,
        ...
        # self.D_detail = Generator2(latent_dim=self.n_detail+self.n_cond, au_dim=41, out_channels=1, out_scale=model_cfg.max_z, sample_mode = 'bilinear').to(self.device)
        self.D_detail = Generator4().to(self.device)
        ...
        if os.path.exists(model_path):
            ...
            print("E_flame loaded")
            util.copy_state_dict(self.E_detail.state_dict(), checkpoint['E_detail'])
            print("E_detail loaded")
            util.copy_state_dict(self.D_detail.state_dict(), checkpoint['D_detail'])
            # util.copy_state_dict(self.D.state_dict(), checkpoint['D'])
            # util.copy_state_dict(self.AUNet.state_dict(), checkpoint['AUNet'])
            # util.copy_state_dict(self.emoca.state_dict(), checkpoint['emoca'])
        else:
    ...
    # @torch.no_grad()
    def encode(self, images, batch, use_detail=True):
        ...
        if use_detail:
            ...
            # print('use detail')
            try : 
                # codedict = self.emoca.encode(batch=batch, training=False)
                raise Exception("forced exception")
                # codedict['detailcode'],_ = self.E_detail(images)
            except : 
                with torch.no_grad():
                    parameters = self.E_flame(images)
                codedict = self.decompose_code(parameters, self.param_dict) # ... parameters are loaded into codedict
                _,codedict['exp'] = self.emoca.encode(batch=batch, training=False)
                # with torch.no_grad():
                with torch.no_grad():
                    detailcode = self.E_detail(images)
                # print("detailcode shape: ", detailcode.shape)
                codedict['detailcode'] = detailcode
                # codedict['detailcode'] = self.update_detail_codes(codedict['detailcode'], self.cfg.dataset.batch_size, self.cfg.dataset.K)
                # codedict['detailcode'] = detailcode
        codedict['images'] = images #codedict images as images
        # x,afn,_= self.AUNet(images, use_gnn=True) # afn : [16,27,512] #codedict ['exp'] shape is [16,50]
        # x,afn,_ =self.AUNet(images)
        # codedict['afn']=(afn>=0.3).int()
        # codedict['AU_features']=x
            # codedict['afn'] = self.AU_Encoder(afn)
            # codedict['detail_features'] = self.E_detail(images)
            # codedict['detailcode'] = self.DAT(codedict['AU_features'], codedict['detail_features'])
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        if use_detail:
            # codedict['encoded_AUF'] = self.AU_Encoder(codedict['AU_features'])
            # uv_z = self.D_detail(torch.cat([codedict['afn'], codedict['posecode'][:,3:], codedict['expcode'], codedict['detailcode'], codedict['encoded_AUF']], dim=1))
            # uv_z = self.D_detail(torch.cat([codedict['afn'], codedict['posecode'][:,3:], codedict['expcode'], codedict['detailcode']], dim=1))
            # uv_z = self.D_detail(torch.cat([codedict['pose'][:,3:], codedict['exp'], codedict['detailcode']], dim=1),codedict['afn'])
            # uv_z = self.D_detail(codedict['detailcode'],codedict['afn'])
            if codedict['detailcode'].shape != [self.cfg.dataset.batch_size,128,1,1]:
                codedict['detailcode'] = codedict['detailcode'].unsqueeze(-1).unsqueeze(-1).expand(-1,-1,1,1)
            uv_z = self.D_detail(torch.cat([codedict['pose'][:,3:].unsqueeze(-1).unsqueeze(-1).expand(-1,-1,1,1), codedict['exp'].unsqueeze(-1).unsqueeze(-1).expand(-1,-1,1,1), codedict['detailcode']], dim=1)) # v1
            # uv_z = self.D_detail(torch.cat([codedict['pose'][:,3:], codedict['exp'], codedict['detailcode']], dim=1))
            if iddict is not None:
                print("iddict is not None")
        ...
        if return_vis:
            ...
            return opdict, visdict['shape_detail_images']
