...
class AU_Feature_Loss(nn.Module):
    ...
    def forward(self, input_image, target_image):
        ...
        for i in range(BatchSize):    
            ...
            #         continue
            #     else :
            #         input_features[1][0][i] = input_features[1][0][i] * self.map_threshold(input_features[1][0][i])
            #         target_features[1][0][i] = target_features[1][0][i] * self.map_threshold(target_features[1][0][i])
            ...
            src_dimp_score = input_features[1][0][11] + input_features[1][0][39] + input_features[1][0][40] #AU 14 dimpler
            # src_dimp_score = src_dimp_score * self.map_threshold(src_dimp_score)
            tar_dimp_score = target_features[1][0][11] + target_features[1][0][39] + target_features[1][0][40] #AU 14 dimpler AU14 : 11 / AUL14 : 40 / AUR14: :41
...
# from ..models.OpenGraphAU.model.MEFL import MEFARG
# class AU_Feature_Loss(nn.Module):
#     def __init__(self, num_main_classes = 27, num_sub_classes = 14, backbone='resnet50'):
#         super(AU_Feature_Loss, self).__init__()
#         self.aufeat = MEFARG(num_main_classes, num_sub_classes, backbone)
#         self.criterion = nn.MSELoss()

#     def forward(self, input_image, target_image):
#         input_features = self.aufeat(input_image)[1]
#         target_features = self.aufeat(target_image)[1]
#         loss = self.criterion(input_features, target_features)
#         return loss
...
class IDMRFLoss(nn.Module):
    ...
    def compute_relative_distances(self, cdist):
        epsilon = 1e-5
        div = torch.min(cdist.clamp(min=0, max=0.5), dim=1, keepdim=True)[0]
...
#         scaled_dist = relative_dist
#         # scaled_dist = torch.clamp(scaled_dist, max=8.872e+01)
#         dist_before_norm = torch.exp(((self.bias - scaled_dist)/self.nn_stretch_sigma).clamp(min=-20, max=20))
#         self.cs_NCHW = self.sum_normalize(dist_before_norm)
#         return self.cs_NCHW
