...
class VGG19FeatLayer(nn.Module):
    def __init__(self):
        ...
        self.vgg19 = models.vgg19(pretrained=True).features.eval().to('cuda:1')
        self.mean = torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1).to('cuda:1')
...
# class IDMRFLoss_v2(nn.Module):
#     def __init__(self, featlayer=VGG19FeatLayer):
#         super(IDMRFLoss_v2, self).__init__()
#         self.featlayer = featlayer()
#         self.feat_style_layers = {'relu1_2' : 1.0, 'relu2_2' : 1.0, 'relu3_3': 1.0, 'relu4_3': 1.0}
#         self.feat_content_layers = {'relu3_3': 1.0}
#         self.bias = 1.0
...
#         scaled_dist = torch.clamp(scaled_dist, max=8.87228355e+01)
...
#         # TODO : VGG 16 feature layer from "A unified embedding for face recognition and clustering." relu3_3 L2 distance feature vecotrs
#         # out = loss(gen_vgg_feats['relu3_3': 1.0],tar_vgg_feats['relu3_3': 1.0])
        
#         # style_loss_list = [self.feat_style_layers[layer] * loss(gen_vgg_feats[layer], tar_vgg_feats[layer]) for layer in self.feat_style_layers]
#         # self.style_loss = reduce(lambda x, y: x+y, style_loss_list) * self.lambda_style
#         # out = self.style_loss
        
#         content_loss_list = [self.feat_content_layers[layer] * loss(gen_vgg_feats[layer], tar_vgg_feats[layer]) for layer in self.feat_content_layers]
...
class IDMRFLoss(nn.Module):
    ...
    def exp_norm_relative_dist(self, relative_dist):
        scaled_dist = relative_dist
        # scaled_dist = torch.clamp(scaled_dist, max=10)
        dist_before_norm = torch.exp(((self.bias - scaled_dist)/self.nn_stretch_sigma).clamp(min=-40, max=40))
        self.cs_NCHW = self.sum_normalize(dist_before_norm)
        return self.cs_NCHW
...
class PerceptualLoss(nn.Module):
    def __init__(self):
        ...
        self.vgg = vgg16(pretrained=True).features[:16].to('cuda:1').eval()
        for param in self.vgg.parameters():
            param.requires_grad = False

    def forward(self, input_image, target_image):
        x_vgg, y_vgg = self.vgg(input_image), self.vgg(target_image)
        loss = torch.nn.functional.mse_loss(x_vgg, y_vgg)
        return loss
...
#     def load_weights(self, path="data/face_recognition_model/vgg_face_torch/VGG_FACE.t7"):
...
#         self.feat_style_layers = {'relu3_3': 1.0}
#         self.feat_content_layers = {'relu3_3': 1.0}
#         self.bias = 1.0
#         self.nn_stretch_sigma = 0.5
#         self.lambda_style = 1.0
