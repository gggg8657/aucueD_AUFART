...
class Trainer(object):
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        self.mymodel.train()
        self.mymodel.TDC.eval()
        self.mymodel.AU_Encoder.eval()
        # self.mymodel.AU_Encoder_disp.train()
        # self.mymodel.AU_Encoder_disp.train()
        ...
        with torch.no_grad(): uv_z_b = self.mymodel.D_detail(torch.cat([posecode[:,3:], expcode, detailcode], dim=1))
        ub_z_au = self.mymodel.AU_D_detail(codedict['afn'])
        uv_z = uv_z_b + ub_z_au
        # render detail
        ...
        #mask patch -> none facial space to zero
            ...
            for step in tqdm(range(iters_every_epoch), desc=f"Epoch[{epoch+1}/{self.cfg.train.max_epochs}]"):
                ...
                if self.global_step % self.cfg.train.log_steps == 0:
                    ...
                    if self.global_step % self.cfg.train.vis_steps == 0 or step == (iters_every_epoch-1):
                        ...
                        grid_image = util.visualize_grid(visdict, savepath, return_gird =True) #save images, visdict => images.
                        # import ipdb; ipdb.set_trace()                    
