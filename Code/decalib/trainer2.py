...
class Trainer(object):
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        ...
        self.mymodel.AU_Encoder.train()
        self.mymodel.AU_Detector.train()
        ...
        if not self.train_detail:
            ...
            if self.cfg.loss.weightedAU:
                ...
                img_aunet = self.AU_net(images)
                rend_aunet = self.AU_net(opdict['rendered_images'])
                image_au = img_aunet[1]
                rend_au = rend_aunet[1]
                pred_au = codedict['pred_au']
                losses['au_pred_loss'] = F.mse_loss(image_au, pred_au)
                # losses['au_loss'] = F.mse_loss(img_aunet[2], rend_aunet[2]) * 27.
                opdict['au_img'] = image_au
                opdict['au_rend'] = rend_au
                losses['au_lmk_loss'] = lossfunc.weighted_au_landmark_loss(predicted_landmarks_dense, lmk_dense, (image_au >= 0.5).float(), self.au_weight) * self.cfg.loss.mainAU
            ...
            losses['reg_jawpose'] = (torch.sum(codedict['pose'][:,3:]**2)/2)*self.cfg.loss.reg_jaw_pose
            losses['reg_expression'] = (torch.sum(codedict['exp']**2)/2)*self.cfg.loss.reg_exp
            if self.cfg.loss.reg_pose > 0.:
                losses['headpose_reg'] = (torch.sum(codedict['pose'][:,:3]**2)/2)*self.cfg.loss.reg_pose
            if self.cfg.loss.reg_cam > 0.:
                losses['cam_reg'] = (torch.sum(codedict['cam'][:,:3]**2)/2)*self.cfg.loss.reg_cam
            ...
            if self.cfg.model.jaw_type == 'euler':
                # import ipdb; ipdb.set_trace()
                # reg on jaw pose
                losses['reg_jawpose_roll'] = (torch.sum(codedict['euler_jaw_pose'][:,-1]**2)/2)*100. * self.cfg.loss.reg_jaw_pose
                losses['reg_jawpose_close'] = (torch.sum(F.relu(-codedict['euler_jaw_pose'][:,0])**2)/2)*10. * self.cfg.loss.reg_jaw_pose
                    ...
                    if self.global_step % self.cfg.train.vis_steps == 0 or step == (iters_every_epoch-1):
                        ...
                        if self.vis_au:
                            visdict = {
                                ...
                                'rendered_images': opdict['rendered_images'][visind]
                                # 'predicted_images': opdict['predicted_images'][visind],
                                # 'au_comp': util.visualize_arrays(opdict['images'][visind], opdict['au_img'], opdict['au_rend'])
                            }
                        else:
                            visdict = {
                                # 'inputs': opdict['images'][visind],
