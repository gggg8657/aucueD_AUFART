...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        shapedirs = torch.cat([shapedirs[:,:,:100], shapedirs[:,:,300:300+config.n_exp]], 2)
        ...
        # default_eyball_pose = torch.ones([1, 6], dtype=self.dtype, requires_grad=False)*0.2
        ...
        mp_lmk_embedding = np.load(config.mp_lmk_embedding_path, allow_pickle=True, encoding='latin1')
        # print(mp_lmk_embedding.__dict__)
        self.register_buffer('mp_lmk_faces_idx', torch.from_numpy(mp_lmk_embedding['lmk_face_idx'].astype(int)).to(torch.int64))
        self.register_buffer('mp_lmk_bary_coords', torch.from_numpy(mp_lmk_embedding['lmk_b_coords']).to(self.dtype).float())
    ...
    def forward(self, shape_params=None, expression_params=None, pose_params=None, eye_pose_params=None):
        ...
        mp = vertices2landmarks(vertices, self.faces_tensor,
                                         mp_lmk_faces_idx,
                                         mp_lmk_bary_coords)
        return vertices, landmarks2d, landmarks3d, mp
...
class FLAMETex(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.tex_type == 'BFM':
            ...
            tex_space = np.load('/home/cine/DJ/DECA/Code/data/BalanceAlb_model.npz')
