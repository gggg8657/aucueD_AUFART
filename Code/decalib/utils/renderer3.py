...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename,flame_mask_file, uv_size=256, rasterizer_type='pytorch3d'):
        ...
        self.flame_mask_file = open(flame_mask_file, 'rb')
        self.flame_mask = pickle.load(self.flame_mask_file, encoding='latin1')
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, h=None, w=None, light_type='point', background=None):
        ...
        # print("albedo_images shape:", albedo_images.shape)
        ...
        v_colors = torch.zeros((batch_size, 5023, 3))
        face_idx = np.union1d(self.flame_mask['face'], self.flame_mask['forehead'])
        v_colors[:, face_idx, :] = 1.0
        # v_colors = v_colors.to('cuda:1')
        v_colors = v_colors.to(rendering.device)
        v_colors = self.render_colors(transformed_vertices, v_colors, h, w).detach()
    ...
    def render_shape(self, vertices, transformed_vertices, colors = None, images=None, detail_normal_images=None, 
        ...
        alpha_images_full = alpha_images*pos_mask
        alpha_images_face = alpha_images*v_colors
        if images is None:
            shape_images_full = shaded_images*alpha_images_full + torch.zeros_like(shaded_images).to(vertices.device)*(1-alpha_images_full)
            shape_images_face = shaded_images*alpha_images_face + torch.zeros_like(shaded_images).to(vertices.device)*(1-alpha_images_face)
        else:
            shape_images_full = shaded_images*alpha_images_full + images*(1-alpha_images_full)
            shape_images_face = shaded_images*alpha_images_face + images*(1-alpha_images_face)
        if return_grid:
            uvcoords_images = rendering[:, 12:15, :, :]; 
            grid = (uvcoords_images).permute(0, 2, 3, 1)[:, :, :, :2]
            return shape_images_full, shape_images_face, normal_images, grid, alpha_images
        else:
            return shape_images_full, shape_images_face
    ...
    def render_colors(self, transformed_vertices, colors, h, w):
        '''
