...
def main(args):
    ...
    for line in lines:
        ...
        # load test images
        testdata = datasets.TestData(inputpath, iscrop=args.iscrop,crop_size=deca_cfg.dataset.image_size, scale=1.05,
        ...
        print(datetime.now())
        for i in tqdm(range(0, len(testdata), 10)):
            starttime = time.time()
            data = testdata[i]
            name = data['imagename']
            ...
            #     opdict_deca, visdict_deca = gate.decode_deca(codedict_deca, use_detail=args.useDetail) #tensor
                emoca_image_path = os.path.join('/mnt/hdd/emoca/DISFA_FULL/',pathname,'rendered_image', name+'.png')
                # emoca_image_path = os.path.join(inputpath, name+'.png')
                emoca = cv2.imread(emoca_image_path) / 255.; emoca = torch.tensor(emoca).unsqueeze(0).float().to(device); emoca = emoca.permute(0,3,1,2)
                compare = [0,1,2,3,5,6,7,9,12,17,19,22]
                image_au = AU_net(images_224)[1][:,compare]
                rend_au = AU_net(emoca)[1][:,compare]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--inputpath', default='./paths_disfaall.txt', type=str,
    ...
    parser.add_argument('-s', '--savefolder', default='videoResult/emoca_DISFAall/', type=str,
                        help='path to the output directory, where results(obj, txt files) will be stored.')
    # parser.add_argument('--videoName', default='resultWithoutTC.avi', type=str,)
    parser.add_argument('--pretrained_modelpath', default='/mnt/hdd/EncoderTrainingCode/Code/Training/testGATE30/model.tar', type=str, help='model.tar path')
