...
#/mnt/hdd/EncoderTrainingCode/Code/decalib/gatfarec.py
#/home/cine/DJ/DECA/demos/demo_reconstruct_emoca_try.py


# 이후, gatfarec 모듈을 사용하여 필요한 기능을 사용할 수 있습니다.
# 예: gatfarec.some_function()
...
from decalib.deca_AU_enc_dec import DECA as DECA_
...
import argparse
from gdl_apps.EMOCA.utils.io import save_obj, save_images, save_codes, test
from decalib.models.OpenGraphAU.model.ANFL import AFG
...
from Code.decalib.gatfarec import DECA as gatfarec
from Code.decalib.utils.config import cfg as aufart_cfg
# gatfarec.py 파일이 있는 디렉토리 경로를 추가합니다.
# sys.path.append('/mnt/hdd/EncoderTrainingCode/Code/decalib')

# gatfarec 모듈을 import 합니다.
# from decalib.gatfarec import DECA as AUFART
...
def main(args):
    ...
    origin_vs_deca_score =[]# masks * images vs  masks * visdict['Deca_detail']
    origin_vs_emc_score =[]
    origin_vs_my_score =[]
    ...
    AUnet = AFG(num_main_classes=auconf.num_main_classes, num_sub_classes=auconf.num_sub_classes, backbone=auconf.arc).to(device)
    AUnet = load_state_dict(AUnet, auconf.resume).to(device)
    AUnet.eval()
    ...
    aufart_cfg.rasterizer_type = "pytorch3d"
    ...
    AUFART = gatfarec(config =aufart_cfg, device=device )
    ...
    for line in lines:
        ...
        savefolder = os.path.join(args.savefolder, pathname)
        os.makedirs(savefolder, exist_ok=True)
        print(savefolder)
        ...
        for i in tqdm(range(0, len(testdata), 1)):
            ...
            cond=(AU_net(images)[1]>=0.5).int().float()
            if torch.all(cond==0):
                print(cond)
                print("\n\n\n\nSKIP!!!!!\n\n\n")
                continue
            with torch.no_grad():
                ...
                a, visdict_emo = test(emoca, batch)
                
                phemc = (a['uv_texture_gt'] -a['uv_texture']).abs()
                phemc_score_list.append(phemc)
                ...
                codedict_AUFART = AUFART.encode(images)
                codedict_AUFART['detail'] = codedict['detail']
                codedict['shape']=codedict_AUFART['shape']
                codedict['pose']= codedict_AUFART['pose']
                codedict['exp']= codedict_AUFART['exp']
                ...
                visdict['EMOCA_detail'] = visdict_emo['output_images_detail']
                visdict2['emoca'] = visdict_emo['geometry_detail']
                opdict, visdict['shape_detail_images'], visdict2['My'], ph_score = deca.decode(codedict) #tensor
                masks = a['masks']
                visdict['Deca_detail'] = (1. - masks) * images + masks * visdict['Deca_detail']
                visdict['shape_detail_images'] = (1. - masks) * images + masks * visdict['shape_detail_images']
                
                deca_face_region = masks * visdict['Deca_detail']
                emoca_face_region = masks * visdict['EMOCA_detail']
                my_face_region = masks * visdict['shape_detail_images']
                origin_face_region = masks * images

                origin_vs_deca_score.append((origin_face_region - deca_face_region).mean())
                origin_vs_emc_score.append((origin_face_region - emoca_face_region).mean())
                origin_vs_my_score.append((origin_face_region - my_face_region).mean())
                ...
                image_au = (AU_net(visdict['inputs'])[1][:, compare]>=0.5).int().float()
                ...
                AUC_vs_deca.append(torch.mean(AUnet(visdict['inputs'])[1] - AUnet(visdict['Deca_detail'])[1])**2)
                AUC_vs_emoca.append(torch.mean(AUnet(visdict['inputs'])[1] - AUnet(visdict['EMOCA_detail'])[1])**2)
                AUC_vs_my.append(torch.mean(AUnet(visdict['inputs'])[1] - AUnet(visdict['shape_detail_images'])[1])**2)

                # AUC_vs_emoca.append(criterion(AUnet(visdict['inputs']) - AUnet(visdict['EMOCA_detail'])))
                # AUC_vs_my.append(criterion(AUnet(visdict['inputs']) - AUnet(visdict['shape_detail_images'])))
    ...
    # ph_min3 = min(phemc_score_list)
    # ph_max3 = max(phemc_score_list)
    # average3 = sum(phemc_score_list) / len(phemc_score_list)
    # phemc_score_list.append(ph_min3)
    # phemc_score_list.append(ph_max3)
    # phemc_score_list.append(average3)
    ...
    #     # print(item.item()+"\n")
    # file.close()
    file = open('TestSamples/examples/for_PSNR_LPIPS_lessdata4/AUC_vs_deca.txt','w')
    for it in AUC_vs_deca:
        it = it.item()
        file.write(str(it)+"\n")
        # print(item.item()+"\n")
    file.close()
    file = open('TestSamples/examples/for_PSNR_LPIPS_lessdata4/AUC_vs_emoca.txt','w')
    for it in AUC_vs_emoca:
        it = it.item()
        file.write(str(it)+"\n")
        # print(item.item()+"\n")
    file.close()
    file = open('TestSamples/examples/for_PSNR_LPIPS_lessdata4/AUC_vs_my.txt','w')
    for it in AUC_vs_my:
        it = it.item()
        file.write(str(it)+"\n")
        # print(item.item()+"\n")
    file.close()
    file = open('TestSamples/examples/for_PSNR_LPIPS_lessdata4/origin_vs_deca.txt','w')
    for it in origin_vs_deca_score:
        it = it.item()
        file.write(str(it)+"\n")
        # print(item.item()+"\n")
    file.close()
    file = open('TestSamples/examples/for_PSNR_LPIPS_lessdata4/origin_vs_emc.txt','w')
    for it in origin_vs_emc_score:
        it = it.item()
        file.write(str(it)+"\n")
        # print(item.item()+"\n")
    file.close()
    file = open('TestSamples/examples/for_PSNR_LPIPS_lessdata4/origin_vs_my.txt','w')
    for it in origin_vs_my_score:
        it = it.item()
        file.write(str(it)+"\n")
        # print(item.item()+"\n")
    file.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--savefolder', default='TestSamples/examples/au_ONLY_DECODER_ADDED_on_AUFART', type=str,
